...
class StdoutLogger:
    ...
    @staticmethod
    def log_dict_as_scalars(step, scalars_dict):
        for name, value in scalars_dict.items():
            StdoutLogger.log_scalar(name, step, value)
...
def log_scalar(name, step, value, log_stdout=True):
    """Logs a scalar to the loggers."""
    if log_stdout:
        allowed_loggers = _loggers
    else:
        allowed_loggers = list(filter(lambda x: x != StdoutLogger, _loggers))

    for logger in allowed_loggers:
        logger.log_scalar(name, step, value)


def log_figure(name, fig):
    """Logs a figure/plot to the loggers."""
    for logger in _loggers:
        logger.log_figure(name, fig)
...
def log_dict_as_scalars(step, scalars_dict):
    """Logs a property to the loggers."""
    for logger in _loggers:
        logger.log_dict_as_scalars(step, scalars_dict)
...
class MetricsAccumulator:
    """Accumulates metrics to average."""
